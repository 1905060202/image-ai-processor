...
router.patch('/images/:filename', async (req, res) => {
  ...
  try {
      ...
      // --- 关键修复：尝试回滚数据库 START ---
      // 如果文件操作失败，但数据库已经修改了，我们需要把数据库改回去
      console.log('尝试回滚数据库记录...');
      await imageManager.updateEntry(finalNewFilename, oldFilename).catch(rollbackError => {
          console.error('数据库回滚失败!', rollbackError);
          // 这里可以记录一个更严重的系统日志，通知管理员需要手动干预
      });
      // --- 关键修复：尝试回滚数据库 END ---
